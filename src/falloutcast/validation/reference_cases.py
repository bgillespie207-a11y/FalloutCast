...
separately, two ways:
  * `idealized_pattern.py` -- BOTH Tier-0 (WSEG-10) and Tier-1 (multi-layer
    advection) vs Glasstone & Dolan's idealized 1-Mt surface-burst reference (a
    true HOB=0, strategic-yield, given-wind pattern matching this model's
    assumptions). Both land within a factor of ~2 downwind across the 1000..1
    R/hr contours -- establishes footprint size is right against the canonical
    idealized reference for both engines.
...
The rest of the harness tests are code-correctness checks (the plumbing runs and
returns finite, structurally-bounded output).
"""
